...
CardPuter86 exposes a virtual Hayes-style modem on `COM1` (`0x3F8`). DOS software sees an 8250-like serial port, while the ESP32-S3 handles Wi-Fi and TCP.

Configure Wi-Fi from POST Settings: after the SD check, hold `Ctrl`, open `Wi-Fi settings`, scan for networks, select an SSID, then enter the password. The credentials are stored in NVS. The separate `COM1 Hayes modem` Settings page only shows modem status and DOS usage hints, keeping Wi-Fi connectivity separate from serial modem emulation for future Wi-Fi file import features. Inside DOS, run `MODEM.COM` to test the COM1 path; it sends `AT` and should print the echoed command plus `OK`.

From a DOS terminal program, use commands such as:
...
This targets Telnet/BBS/MUD-style software and simple serial programs. It is not a packet driver or a full DOS TCP/IP stack.
