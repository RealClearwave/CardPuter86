...
PC Speaker audio is generated by a FreeRTOS task pinned to Core 0, then written to I2S DMA in 128-frame batches. The default `cardputer86.img` removes the old bundled games and includes `CP86TEST.COM`, `MININASM.COM`, `TERM.COM`, `TE.COM`, `MMLPLAY.COM`, and `PHANTOM.MML`.
