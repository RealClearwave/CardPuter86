...
PC Speaker audio is generated by a FreeRTOS task pinned to Core 0, then written to I2S DMA in 128-frame batches. The default `cardputer86.img` is built from `tools/dos/image_root` and includes redistributable Svarog86/FreeDOS files, `CP86TEST.COM`, `MININASM.COM`, basic tools, and `PHANTOM.MID`.
