...
Configure the Wi-Fi SSID/password from POST Settings: after the SD check, hold `Ctrl`, then open `Wi-Fi modem`. The credentials are stored in NVS. Inside DOS, run `MODEM.COM` to test the COM1 path; it sends `AT` and should print the echoed command plus `OK`.
