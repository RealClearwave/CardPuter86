...
Las teclas normales del Cardputer corresponden a las teclas del PC. La tecla Aa corresponde a Shift, y Aa+`1234567890-= envia `~!@#$%^&*()_+`. Ctrl y Alt funcionan como modificadores. Fn es la capa de funciones de CardPuter86: Fn+1 hasta Fn+0 envian F1-F10, Fn+- envia F11, Fn+= envia F12, Fn+` envia Esc, Fn+Retroceso envia Delete, y Fn+;, Fn+,, Fn+. y Fn+/ se envian directamente al emulador como cursores del PC. Opt+Espacio cambia entre el modo de texto legible con celdas 6x8 y el modo escalado. En modo de texto, Opt+;, Opt+,, Opt+. y Opt+/ desplazan la vista 40x16; Opt+' devuelve una vista FIXED a su posición inicial superior izquierda. AUTO sigue la última línea de contenido y mantiene fijadas hasta dos líneas de estado detectadas al pie. El modo escalado usa una fuente 3x5 para las pantallas de texto y escala las pantallas gráficas a toda la LCD. Fn+Espacio pausa el emulador y abre Settings. G0 no se usa para la gestion de energia del emulador.
