...
Wi-Fi 信息在 Settings 的 `WiFi modem` 中配置：固件会扫描附近热点，选择 SSID 后输入密码，并保存到 NVS。modem 支持最小 AT 命令集：`AT`、`ATI`、`ATE0/ATE1`、`ATZ`、`AT+WIFI?`、`ATDT host:port`、`+++`、`ATH`。`AT+WIFI?` 只查看状态；SSID 和密码通过 Settings 扫描流程配置，不通过 AT 命令手输。`CP86TEST.COM` 包含直接访问 COM1 的 `AT` 探测。
...
POST 阶段按住 `Alt` 才会探测并挂载 SD 卡；未插卡时不会阻塞启动。模拟器运行后按 `Fn+空格` 暂停并进入 Settings，标题栏会显示当前 RTC 时间和电池电量。Settings 提供持久化的“USB 接口作用”：仅充电、串口 CDC、USB disk；串口和 USB 磁盘是互斥的活动模式。
