...
电源设置可关闭自动休眠，或设为 1、2、5、10、30、60 分钟；默认 2 分钟。休眠使用 ESP32-S3 deep sleep，并用 G0 这个 RTC GPIO 作为唤醒源；没有启用 ULP 键盘矩阵扫描，因为持续扫描键盘会增加待机功耗。Settings 还提供模拟 RTC 时钟设置，DOS 可通过标准 CMOS RTC 端口 `70h/71h` 读取。
