...
电源设置可设为 30 秒、2 分钟、5 分钟、10 分钟或永不；默认 2 分钟。休眠使用 ESP32-S3 light sleep，并关闭 LCD 面板与背光；固件每 100ms 短暂唤醒扫描 Cardputer 键盘矩阵，按任意键恢复显示。G0 不再用于休眠或唤醒。Settings 还提供模拟 RTC 时钟设置，DOS 可通过标准 CMOS RTC 端口 `70h/71h` 读取。
