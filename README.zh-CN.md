...
```sh
./flash.sh --package --version 0.3.7
```
...
Wi-Fi 在 POST Settings 里配置：SD 检查后按住 `Ctrl`，进入 `Wi-Fi settings`，扫描网络、选择 SSID、输入密码。凭据会保存到 NVS。单独的 `COM1 Hayes modem` 页面只显示串口 modem 状态和 DOS 用法提示，使 Wi-Fi 连接能力与串口 modem 模拟解耦，便于后续加入 Wi-Fi 文件导入。进入 DOS 后可运行 `MODEM.COM` 测试 COM1；它会发送 `AT`，正常应显示回显的 `AT` 和 `OK`。
