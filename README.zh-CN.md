...
电源设置可关闭自动休眠，或设为 1、2、5、10、30、60 分钟；默认 2 分钟。当前休眠采用关屏待机：关闭 LCD 面板与背光，然后按任意键或 G0 恢复显示。GPIO0 同时是 Cardputer 的 Boot strap 脚，若作为 deep sleep 唤醒源，按住 G0 唤醒时容易进入下载模式，因此不能可靠用于 G0 deep sleep 唤醒。Settings 还提供模拟 RTC 时钟设置，DOS 可通过标准 CMOS RTC 端口 `70h/71h` 读取。
