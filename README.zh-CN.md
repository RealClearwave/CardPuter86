...
默认 `cardputer86.img` 现在由 `tools/dos/build_image.py` 从 `tools/dos/image_root` 目录生成。公开镜像只放可再分发内容：Svarog86/FreeDOS 启动文件、`CP86TEST.COM`、`MININASM.COM`、小型编辑器、基础 DOS 工具，以及用户提供的 `PHANTOM.MID` 示例。`CP86TEST.COM` 每项测试都会询问 `Run? [Y/n]`，可跳过耗时或依赖硬件状态的项目。
...
如需更新默认镜像，直接修改 `tools/dos/image_root`，然后运行 `python3 tools/dos/build_image.py --prepare-source`。`flash.sh` 在构建固件和 M5Burner 包之前会自动执行这一步。
