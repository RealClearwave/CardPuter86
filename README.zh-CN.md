...
- `Ctrl`、`Alt`：对应的 PC 修饰键
- `Opt+空格`：切换文字模式与缩放模式
- `Opt+;`、`Opt+,`、`Opt+.`、`Opt+/`：在文字模式中滚动 40×16 视口上、左、下、右
- `Opt+'`：FIXED 模式回到左上初始位置
- `Fn+;`、`Fn+,`、`Fn+.`、`Fn+/`：全局方向键上、左、下、右，直接传给模拟器
...
文字视口默认处于 AUTO 模式，自动跟随正文最后一行，并尝试识别和固定底部最多两行状态栏。使用任意 Opt 方向组合后进入 FIXED 模式，不再自动滚动。

## 音频

PC Speaker 音频由固定在 Core 0 的 FreeRTOS 任务生成，并以 128 个立体声帧为一批写入 I2S DMA。这样 PIT 2 方波不再依赖显示刷新节奏，8086 模拟主循环仍保留在 Arduino 核心。

默认 `cardputer86.img` 以原 tinyfake86 兼容系统盘为基础，删除了内置游戏，并加入 `CP86TEST.COM`、`MININASM.COM`、`TERM.COM`、`TE.COM` 和 Shiru 的 `SB.COM` System Beeps PC speaker musicdisk。在 DOS 下运行 `CP86TEST` 可统一测试 RTC、BIOS 时钟 tick、磁盘枚举、键盘、扬声器、COM1 modem、Wi-Fi modem 状态和 USB 模式提示；运行 `TERM` 可直接和 COM1 Hayes modem 交互；运行 `TE FILE.TXT` 可使用 40×14 缓冲的 nano 风格小编辑器；运行 `SB` 可播放 System Beeps PC speaker demo。系统盘内附带 `SB.TXT`，包含原作者署名和 CC BY 3.0 授权说明。
