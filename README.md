...
The default `cardputer86.img` is now built from `tools/dos/image_root` by `tools/dos/build_image.py`. It contains redistributable Svarog86/FreeDOS boot files, `CP86TEST.COM`, `MININASM.COM`, a small editor, basic DOS utilities, and the user-provided `PHANTOM.MID` sample. `CP86TEST.COM` asks `Run? [Y/n]` before each hardware test.
...
Run `python3 tools/dos/build_image.py --prepare-source` to rebuild the default image from the source directory. `flash.sh` runs this step automatically before firmware and M5Burner packaging builds.
