...
The default `cardputer86.img` is based on the original tinyfake86-compatible system disk with the bundled games removed. It now includes `CP86TEST.COM`, `MININASM.COM`, `TERM.COM`, `TE.COM`, and `MUSIC.COM`. Run `CP86TEST` for the all-in-one RTC, BIOS clock, disk, keyboard, PC speaker, COM1 modem, Wi-Fi modem status, and USB-mode note tests. Run `TERM` for direct COM1 Hayes modem interaction, `TE FILE.TXT` for a 40x14 nano-like tiny editor, and `MUSIC` for an embedded PC speaker music demo.
