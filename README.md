...
The default `cardputer86.img` is based on the original tinyfake86-compatible system disk with the bundled games removed. It now includes `CP86TEST.COM`, `MININASM.COM`, `TERM.COM`, `TE.COM`, `MMLPLAY.COM`, and `PHANTOM.MML`. Run `CP86TEST` for the all-in-one RTC, BIOS clock, disk, keyboard, PC speaker, COM1 modem, Wi-Fi modem status, and USB-mode note tests. Run `TERM` for direct COM1 Hayes modem interaction, and `MMLPLAY PHANTOM.MML` for a PC speaker music demo.
