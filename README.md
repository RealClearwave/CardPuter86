...
The default `cardputer86.img` includes `CP86TEST.COM`. Run it from DOS for the all-in-one RTC, BIOS clock, disk, keyboard, PC speaker, COM1 modem, Wi-Fi modem status, and USB-mode note tests. Each test asks `Run? [Y/n]`, so long or hardware-dependent checks can be skipped.
