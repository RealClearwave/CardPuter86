...
Configure Wi-Fi from Settings with `WiFi modem`; it scans nearby access points, lets you choose an SSID, and stores the password in NVS. The modem implements a small AT command set: `AT`, `ATI`, `ATE0/ATE1`, `ATZ`, `AT+WIFI?`, `ATDT host:port`, `+++`, and `ATH`. `AT+WIFI?` reports status; credentials are configured from Settings rather than typed through AT commands. `CP86TEST.COM` includes a direct COM1 `AT` probe.
